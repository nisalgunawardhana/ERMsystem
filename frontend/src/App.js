...
function App() {
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('token')) {
      ...
      socketConnection.on('connect', () => {
        ...
        startHeartbeat(socketConnection);
      }
      );
  ...
  const startHeartbeat = (socketConnection) => {
    // Send a heartbeat message to the server every 5 seconds
    setInterval(() => {
      socketConnection.emit('heartbeat', 'ping');
    }, 5000);
  };
